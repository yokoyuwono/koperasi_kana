            ...
            <table class="min-w-full text-xs">
                <thead class="bg-slate-50 text-slate-600">
                    <tr>
                        ...
                        <th class="text-right px-4 py-3">Jumlah Deposit</th>
                        <th class="text-right px-4 py-3">Total Komisi</th>
                        <th class="text-left px-4 py-3">Tanggal Approval</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($rows as $r)
                        @php    
                            $unpaid = (int)($r->unpaid_count ?? 0);
                            $isPaid = $unpaid === 0;
                        @endphp
                        <tr class="hover:bg-slate-50/50">
                            <td class="px-4 py-3">
                                <div class="font-semibold text-slate-800">{{ $r->agent_nama }}</div>
                                <div class="text-[11px] text-slate-500">{{ $r->kode_agent }}</div>
                            </td>
                            <td class="px-4 py-3">{{ $r->jabatan }}</td>
                            <td class="px-4 py-3 text-right font-semibold">{{ (int)$r->jumlah_deposit }}</td>
                            <td class="px-4 py-3 text-right font-semibold">{{ $fmt($r->total_komisi) }}</td>
                            <td class="px-4 py-3">{{ $r->approval_min }}</td>
                            <td class="px-4 py-3">
                                @if($isPaid)
            ...
            @forelse($rows as $r)
                <div class="p-4 space-y-2">
                    <div class="flex items-start justify-between gap-2">
                        <div>
                            <div class="text-sm font-semibold text-slate-800">{{ $r->agent_nama }}</div>
                            <div class="text-[11px] text-slate-500">{{ $r->kode_agent }} · {{ $r->jabatan }}</div>
                        </div>
                        <div class="text-right">
                            <div class="text-[11px] text-slate-500">Total</div>
                            <div class="text-sm font-semibold text-slate-800">{{ $fmt($r->total_komisi) }}</div>
                        </div>
                    </div>

                    <div class="text-xs text-slate-600">
                        Deposit: <span class="font-semibold text-slate-800">{{ (int)$r->jumlah_deposit }}</span><br>
                        Approval: <span class="font-medium">{{ $r->approval_min }}</span> s/d <span class="font-medium">{{ $r->approval_max }}</span>
                    </div>
                </div>
            @empty
  ...
  <div class="relative mx-auto mt-20 w-[92%] max-w-xl bg-white rounded-xl shadow-lg border border-slate-100">
    ...
    <form method="POST" action="{{ route('komisi.report.pay') }}" class="p-4 space-y-3">
      ...
      <div class="flex items-center justify-between">
        <div class="text-xs font-semibold text-slate-700">Pilih Agent</div>
          ...
          <label class="flex items-start gap-3 p-3">
            ...
            <div class="min-w-0">
              <div class="text-xs font-semibold text-slate-800 truncate">
                {{ $r->agent_nama }} <span class="text-slate-500 font-normal">({{ $r->kode_agent }})</span>
              </div>
              <div class="text-[11px] text-slate-500">
                {{ $r->jabatan }} · Total: <span class="font-semibold">{{ $fmt($r->total_komisi) }}</span>
                · Status:
                @if($isPaid)
      ...
      @error('agent_ids') <p class="text-[11px] text-red-600">{{ $message }}</p> @enderror
