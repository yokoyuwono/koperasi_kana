...
@section('content')
<div class="mb-4">
    <h2 class="text-xl font-semibold">Edit Data Agen</h2>
    <p class="text-xs text-slate-500 mt-1">
        Perbarui data agen sesuai kebutuhan. Pastikan perubahan sudah sesuai dokumen fisik.
    </p>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5 space-y-6">
    <form 
        ...
        <div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="kode_agent" value="{{ old('kode_agent', $agent->kode_agent) }}"
                ...
                <div>
                    ...
                    <input name="nama" value="{{ old('nama', $agent->nama) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="grid md:grid-cols-3 gap-4 mt-4">
                ...
                <div>
                    ...
                    <select id="jabatan" name="jabatan"
                        ...
                        <option value="BDP" {{ old('jabatan', $agent->jabatan) == 'BDP' ? 'selected' : '' }}>BDP</option>
                        <option value="RM"  {{ old('jabatan', $agent->jabatan) == 'RM'  ? 'selected' : '' }}>RM</option>
                    </select>
        ...
        <div>
            ...
            <div class="grid md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input name="tempat_lahir" value="{{ old('tempat_lahir', $agent->tempat_lahir) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                </div>

                <div>
                    <label class="block text-xs font-medium text-slate-700 mb-1">Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $agent->tanggal_lahir) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="jenis_kelamin"
                        ...
                        <option value="Laki-laki" {{ old('jenis_kelamin', $agent->jenis_kelamin) == 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                        <option value="Perempuan" {{ old('jenis_kelamin', $agent->jenis_kelamin) == 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-4 mt-4">
                <div>
                    ...
                    <input name="NIK" value="{{ old('NIK', $agent->NIK) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat_KTP" rows="2"
                              class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">{{ old('alamat_KTP', $agent->alamat_KTP) }}</textarea>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-4 mt-4">
                <div>
                    ...
                    <input name="telepon" value="{{ old('telepon', $agent->telepon) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat_sekarang" rows="2"
                              class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">{{ old('alamat_sekarang', $agent->alamat_sekarang) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input name="rekening_bank" value="{{ old('rekening_bank', $agent->rekening_bank) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                </div>

                <div>
                    <label class="block text-xs font-medium text-slate-700 mb-1">Tanggal Daftar</label>
                    <input type="date" name="tanggal_daftar" value="{{ old('tanggal_daftar', $agent->tanggal_daftar) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
        ...
        <div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div id="field_referral" class="hidden">
                    ...
                    @php
                        $selectedId = old('refferred_by_agent_id', $agent->refferred_by_agent_id);
                    @endphp
                ...
                <div id="field_atasan" class="hidden">
                    ...
                    @php
                        $selectedAtasan = old('atasan_id', $agent->atasan_id ?? null);
                    @endphp
                    <select name="atasan_id"
                        ...
                        @foreach($refferralAgents as $a)
                            @if($a->jabatan === 'BDP')
                                <option value="{{ $a->id }}" {{ $selectedAtasan == $a->id ? 'selected' : '' }}>
                                    {{ $a->kode_agent }} - {{ $a->nama }}
                                </option>
                            @endif
                        @endforeach
        ...
        <div class="flex justify-end gap-2 pt-3 border-t border-slate-100">
            ...
            {{-- tombol submit hanya untuk ADMIN --}}
            @if(!$isReadOnly)
