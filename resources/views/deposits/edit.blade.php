...
<div class="mb-4 flex flex-col gap-1">
    ...
    <p class="text-xs text-slate-500">
        Komisi yang sudah <strong>approved</strong> tidak bisa diubah.  
        Komisi dengan status <strong>rejected</strong> dapat diperbaiki dan dikirim ulang ke COA.
    </p>
    @if($deposit->status === 'rejected' && $deposit->catatan)
        <div class="mt-1 text-[11px] text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2">
            Alasan penolakan COA: {{ $deposit->catatan }}
        </div>
    @endif
</div>

<div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5 space-y-6 max-w-5xl">
    <form method="POST" action="{{ route('deposits.update', $deposit) }}" enctype="multipart/form-data" class="space-y-6">    
        @csrf
        @method('PUT')
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="select_nasabah" name="id_nasabah"
                        ...
                        @foreach($nasabah as $n)
                            <option value="{{ $n->id }}"
                                {{ old('id_nasabah', $deposit->id_nasabah) == $n->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="select_agent" name="id_agent"
                        ...
                        @foreach($agents as $a)
                            <option value="{{ $a->id }}" data-jabatan="{{ $a->jabatan }}"
                                {{ old('id_agent', $deposit->id_agent) == $a->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <div class="grid md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input name="no_bilyet" value="{{ old('no_bilyet', $deposit->no_bilyet) }}"
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_transaksi" value="{{ old('tanggal_transaksi', optional($deposit->tanggal_transaksi)->format('Y-m-d')) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                ...
                <!-- <div>
                    ...
                    <input type="number" min="1" name="tenor" value="{{ old('tenor', $deposit->tenor) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    <select id="tenor" name="tenor"
                        ...
                        <option value="1"  {{ old('tenor', $deposit->tenor) == 1  ? 'selected' : '' }}>1 Bulan</option>
                        <option value="3"  {{ old('tenor', $deposit->tenor) == 3  ? 'selected' : '' }}>3 Bulan</option>
                        <option value="6"  {{ old('tenor', $deposit->tenor) == 6  ? 'selected' : '' }}>6 Bulan</option>
                        <option value="12" {{ old('tenor', $deposit->tenor) == 12 ? 'selected' : '' }}>12 Bulan</option>
                    </select>
            ...
            <div class="grid md:grid-cols-2 gap-4 mt-3">
                <div>
                    ...
                    <input type="date" id="tanggal_mulai" name="tanggal_mulai" value="{{ old('tanggal_mulai', optional($deposit->tanggal_mulai)->format('Y-m-d')) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                ...
                <!-- <div>
                    ...
                    <input type="date" name="tanggal_tempo" value="{{ old('tanggal_tempo', optional($deposit->tanggal_tempo)->format('Y-m-d')) }}"
                           class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    <input id="tanggal_tempo" type="date" name="tanggal_tempo"
                        value="{{ old('tanggal_tempo', $deposit->tanggal_tempo) }}"
                        readonly
                        class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm
                            focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">

                    <p class="text-[11px] text-slate-500 mt-1">
                        Otomatis dihitung dari tanggal mulai + tenor.
                    </p>
        ...
        {{-- Nominal & BDP --}}
        <div>
            <h3 class="text-sm font-semibold text-slate-700 mb-3">Nominal & BDP Referral</h3>
            <div class="grid md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-xs font-medium text-slate-700 mb-1">Nominal (rupiah) <span class="text-red-500">*</span></label>
                    <input type="number" min="0" name="nominal" value="{{ old('nominal', $deposit->nominal) }}"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="number" step="0.1" min="0"
                                ...
                                value="{{ old('komisi_rm_persen_final', $rmFinalPercent) }}"
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="number" step="0.1" min="0"
                                ...
                                value="{{ old('komisi_bdp_persen_final', $bdpFinalPercent) }}"
                                class="w-24 border border-slate-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                            <span class="text-xs text-slate-500">%</span>
                            <span class="text-[11px] text-slate-500 ml-auto">
        ...
        <div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    @if($deposit->bukti_transfer)
                        <div class="mb-2 text-[11px] text-slate-600">
                            <span class="block mb-1">File saat ini:</span>
                            <a href="{{ asset('storage/'.$deposit->bukti_transfer) }}" target="_blank"
                            class="inline-flex items-center px-2 py-1 rounded bg-slate-100 text-[11px] hover:bg-slate-200">
            ...
            <div class="mt-3">
                ...
                <textarea name="catatan_admin" rows="3"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">{{ old('catatan_admin', $deposit->catatan_admin) }}</textarea>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    if (document.getElementById('select_nasabah')) {
        new TomSelect('#select_nasabah', { create:false, allowEmptyOption:true });
    }
    if (document.getElementById('select_agent')) {
        new TomSelect('#select_agent', { create:false, allowEmptyOption:true });
    }
});
</script>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function recalc() {
        if (!tenorEl.value || !mulaiEl.value) return;

        const startDate = new Date(mulaiEl.value + 'T00:00:00');
        const dueDate = addMonths(startDate, parseInt(tenorEl.value, 10));
    ...
    // initial hitung (penting untuk EDIT)
    recalc();
});
</script>
